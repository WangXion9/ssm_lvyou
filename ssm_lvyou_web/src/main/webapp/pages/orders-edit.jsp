...
<body class="hold-transition skin-blue sidebar-mini">
	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<section class="content">
				<form id="sub" action="${pageContext.request.contextPath}/orders/editOrdersAll.do"  method="post">
					<!--订单信息-->
						...
						<div class="row data-type">
							...
							<div class="col-md-4 data">
								<input type="text" name="orderNum" class="form-control" placeholder="订单编号"
									   value="${orders.orderNum}"  readonly >
							</div>
							...
							<div class="col-md-4 data">
								<div class="input-group date">
									...
									<input type="text" class="form-control pull-right"
										   id="datepicker-a3" name="orderTime" value="${orders.orderTimeStr}" readonly>
								</div>
							</div>
						...
						<div class="row data-type">
							...
							<table id="productlist"
								...
								<%--<c:forEach items="${pageInfo.list}" var="product">--%>
									<%--<tr>--%>
										<%--<td><input name="ids" type="checkbox" checked="checked" value="${product.id}"></td>--%>
										<%--<td>${product.id}</td>--%>
					...
					<div class="panel panel-default">
						...
						<table id="dataList"
							...
							<c:forEach items="${orders.travellers}" var="traveller" varStatus="status">
									<tr id="travellerList">
					...
					<div class="panel panel-default">
						...
						<div class="row data-type">
							<input type="hidden" name="memberId" value="${orders.member.id}"/>

							<div class="col-md-2 title">姓名</div>
							<%--<div class="col-md-4 data text">${orders.member.nickname }</div>--%>
							<div class="col-md-4 data text">
								<input type="text" name="member.name" class="form-control checkName" value="${orders.member.name}" onblur="checkName()">
							</div>
							...
							<div class="col-md-4 data text">
								<input type="text" name="member.nickname" value="${orders.member.nickname}" class="form-control checkNickName" onblur="checkNickName()">
							</div>

							<div class="col-md-2 title">手机号</div>
							<%--<div class="col-md-4 data text">${orders.member.phoneNum}</div>--%>
							<div class="col-md-4 data text">
								<input type="text" name="member.phoneNum" value="${orders.member.phoneNum}" class="form-control checkPhone" onblur="checkPhone()">
							</div>

							<div class="col-md-2 title">邮箱</div>
							<%--<div class="col-md-4 data text">${orders.member.email}</div>--%>
							<div class="col-md-4 data text">
								<input type="text" name="member.email" value="${orders.member.email}" class="form-control checkEmail" onblur="checkEmail()">
							</div>
	...
	<script>
		...
		var travellerSize = ${fn:length(orders.travellers)-1};
        //var travellerSize = travellers.length();
        // alert(travellers);
		...
		function getPage(page) {
            $.get({
                ...
                success:function(data){
					...
					var productId = "${orders.product.id}";
                    ...
                    for (var i = 0; i < length; i++) {
                        if (productId == list[i].id){
                            html+='<tr>\n' +
								'<td><input name="productId" type="radio" class="icheckbox_square-blue" checked="checked" value="${orders.product.id}" ></td>\n' +
        ...
        function selectShow () {
            //回显订单是否支付
            $("#orderStatus").find("option[value='${orders.orderStatus}']").attr("selected",'selected');
            //回显订单支付信息
            $("#payType").find("option[value='${orders.payType}']").attr("selected",'selected');
            //
            $("#payStatus").find("option[value='${orders.payStatus}']").attr("selected",'selected');

            $("#overStatus").find("option[value='${orders.overStatus}']").attr("selected",'selected');
        }
        ...
        function checkName() {
            ...
            if("" == username){
                ...
                var flag;
                $.get({
                    url:"${pageContext.request.contextPath}/member/findMemberByMemberName.do?id=${orders.member.id}&name="+username,
                    dataType:"json",
                    async:false,
                    success:function (data) {
                        if (!data){
        ...
        function saveAll() {
            checkProduct();
            if (checkName()&&checkNickName()&&checkPhone()&&checkEmail()&&checkManNum()&&checkDesc()&&checkSelect()&&checkProduct()){
                $("#sub").submit();
            } else {
                alert("表单请填写完整!");
            }
